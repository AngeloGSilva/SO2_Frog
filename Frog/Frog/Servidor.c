...
int _tmain(int argc, TCHAR* argv[]) {
	...
	//em vez de aqui tem de ser uma thread por cliente a ser criada dentro do while penso eu e so passa o handle do pipe desse cliente e nao todos
	//criacao da thread
	...
	_tprintf(TEXT("[Leitor] Criar uma cópia do pipe '%s' ... (CreateNamedPipe)\n"), PIPE_NAME);
	...
	while (!dados.terminar) {
		...
		_tprintf(TEXT("[LEITOR] Esperar ligação de um leitor... (ConnectNamedPipe)\n"));
	...
	for (int i = 0; i < dados.numClientes; i++) {
		_tprintf(TEXT("[Leitor] Desligar o pipe (DisconnectNamedPipe)\n"));
	...
	//provavelmente nao preciso mais disto
	while (terminar == 0)
	{
